		...
		(function(){
			...
			function mapPreMake(elem) {	
				...
				function makeAlt(href) {
					...
					let objOfElem = {partNum: href[0], id: idOrganize};
					let checkForSameElem = arrWithoutSameElem.find(e => e.partNum === href[0]);
					if (checkForSameElem === undefined){
						//console.log(true)
						arrWithoutSameElem.push(objOfElem);
					}
			...
			function setImgForHoverImg(elem) {
				let imgPartNum = elem.match(regexHTM) != null ?`https://www.badboylawnmowerparts.com/v/vspfiles/photos/${elem.match(regexHTM)[0]}-1.jpg`:'https://www.badboylawnmowerparts.com/v/popUpForDiagram/errorImg.png';
